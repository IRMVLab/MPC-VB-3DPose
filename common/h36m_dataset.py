...
class Human36mDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=True):
        ...
        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_params)        #准备相机属性
        for cameras in self._cameras.values():#把intrinsic里的内容更新添加到extrinsic里
            for i, cam in enumerate(cameras):
                cam.update(h36m_cameras_intrinsic_params[i])    #把intrinsic里的内容更新添加到extrinsic里
                for k, v in cam.items():
                    if k not in ['id', 'res_w', 'res_h']:
                        cam[k] = np.array(v, dtype='float32')   #把list类型变为numpy类型
                
                # Normalize camera frame
                cam['center'] = normalize_screen_coordinates(cam['center'], w=cam['res_w'], h=cam['res_h']).astype('float32')   #归一正则化
                ...
                cam['intrinsic'] = np.concatenate((cam['focal_length'],
                                                   ...
                                                   cam['tangential_distortion']))       #拼接，与投影处按列分离呼应。
        
        # Load serialized dataset
        data = np.load(path,allow_pickle=True)['positions_3d'].item()
        
        self._data = {}
        for subject, actions in data.items():   #加载data，subject主题（S1-S9），action动作
            self._data[subject] = {}    #新建Si（S1-S9）的词条
            for action_name, positions in actions.items():  #迭代action的内容
                self._data[subject][action_name] = {
                    'positions': positions,                 #读入关节位置
                    'cameras': self._cameras[subject],      #加上准备好的相机属性
                }
                
        if remove_static_joints:        #移除部分细节关节
