...
class TemporalModel(TemporalModelBase):
    ...
    def __init__(self, num_joints_in, in_features, num_joints_out, boneindex, temperature, randnumtest, filter_widths, causal=False, dropout=0.25, channels=1024, dense=False):
        ...
        self.shrink_direct_1 = nn.Conv1d(channels, (num_joints_out-1+23)*3, 1)
        self.shrink_direct_2 = nn.Conv1d(channels, (num_joints_out-1+23)*3, 1)
        self.lengthlinear = nn.Linear(num_joints_out-1+23,channels)
        self.directlinear = nn.Conv1d((num_joints_out-1+23)*3, channels, 1)
        self.lengthlinear_1 = nn.Linear(num_joints_out-1+23,channels)
        self.lengthlinear_2 = nn.Linear(num_joints_out-1+23,channels)
        self.directlinear_1 = nn.Conv1d((num_joints_out-1+23)*3, channels, 1)
        self.directlinear_2 = nn.Conv1d((num_joints_out-1+23)*3, channels, 1)
    ...
    def _forward_blocks(self, x):
        ...
        bone_assum = bone[:,:,(self.num_joints_out-1):]
        bone = bone[:,:,:(self.num_joints_out-1)]
        ...
        if not self.causal:
            ...
            bonelength = torch.cat((bonelength,bone_assum[:,self.sinrecfield:-self.sinrecfield].permute(0,2,1)),1)
        # in causal mode, you can only observe the frames before the current one, ranomly sample 'randnumtest' frames to predict the bone lengths
            ...
            bonelength = torch.cat((bonelength,bone_assum[:,self.sinrecfield*2:].permute(0,2,1)),1)
        ...
        for ii in range(self.sinrecfield, xtest.size(2)+self.sinrecfield):
            ...
            xall.append(x)
        x = torch.cat(xall,2)
        bonedirect = x.view(x.size(0),self.num_joints_out-1+23,3,x.size(2))
        bonesum = torch.pow(torch.pow(bonedirect,2).sum(2),0.5).unsqueeze(2)
        bonedirect = bonedirect/bonesum
        bonedirect = bonedirect.view(bonedirect.size(0),(self.num_joints_out-1+23)*3,bonedirect.size(3))
        #compute the final 3D joint locations
        bonelength = bonelength.permute(0,2,1).contiguous().view(-1,self.num_joints_out-1+23)
...
class TemporalModelOptimized1f(TemporalModelBase):
    ...
    def forward(self, x, x_rand, x_randaug):
        ...
        x_rand_boneatt = self.boneatt(x_rand_con.view(bs*ss,-1)).view(x_rand_con.size(0), x_rand_con.size(1), -1) # bs,ss,16
        x_rand_boneatt = x_rand_boneatt * self.temperature
        x_rand_boneatt = self.softmax(x_rand_boneatt)
        bone = getbonelength(x_rand2.detach().view(bs,ss,-1,3), self.boneindex) 
        bonelength = (bone[:,:,:self.num_joints_out-1] * x_rand_boneatt).sum(1) 
        
        #virtual bones prediction
        bs_vir = x_virtual.size(0)
        ss_vir = x_virtual.size(1)
        
        x_virtual = x_virtual[:,int(ss_vir/2)]
        x_virtual = x_virtual.reshape(x_virtual.size(0),-1)
        x_virtual = self.drop(self.relu(self.layers_bnbone[0](self.layers_convbone[0](x_virtual))))
        for i in range(1,len(self.pad)):
            res_virtual = x_virtual
            x_virtual = self.drop(self.relu(self.layers_bnbone[2*i-1](self.layers_convbone[2*i-1](x_virtual))))
            x_virtual = self.drop(self.relu(self.layers_bnbone[2*i](self.layers_convbone[2*i](x_virtual))))
            x_virtual = x_virtual + res_virtual
        x_virtual = self.layers_convbone[-1](x_virtual)
        x_virtual = x_virtual.view(bs_vir,1, self.num_joints_out, 3)
        bone_virtual = getbonelength(x_virtual.detach(), self.boneindex) 
        bone_assum = bone_virtual[:,:,self.num_joints_out-1:].view(bone_virtual.size(0),-1) 
        
        bonelength = torch.cat((bonelength,bone_assum),1)
        ...
        bonelengthaug = (boneaug[:,:,:self.num_joints_out-1] * x_rand_boneattaug).sum(1)
        ...
        bonedirect_1 = bonedirect_1.view(bonedirect_1.size(0),self.num_joints_out-1+23,3)
        bonesum_1 = torch.pow(torch.pow(bonedirect_1,2).sum(2),0.5).unsqueeze(2)
        bonedirect_1 = bonedirect_1/bonesum_1
        bonedirect_1 = bonedirect_1.view(bonedirect_1.size(0),(self.num_joints_out-1+23)*3,1)
        ...
        bonedirect_2 = bonedirect_2.view(bonedirect_2.size(0),self.num_joints_out-1+23,3)
        bonesum_2 = torch.pow(torch.pow(bonedirect_2,2).sum(2),0.5).unsqueeze(2)
        bonedirect_2 = bonedirect_2/bonesum_2
        bonedirect_2 = bonedirect_2.view(bonedirect_2.size(0),(self.num_joints_out-1+23)*3,1)
        ...
        return x, bonelength, x_rand, bonelengthaug, x_randaug, bonedirect_2, bonedirect_1, js_2, js_1, x_virtual
