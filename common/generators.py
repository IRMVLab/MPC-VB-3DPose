...
def getbonedirect(seq, boneindex):  #计算骨骼方向
    bs = np.shape(seq)[0]
    ss = np.shape(seq)[1]
...
'''
def randomaug(batch_3D_rand_ori, boneindex, augdegree):     #更改一些3D真值的骨长，作为新的数据
    batch_3D_rand_ori[:,:,0,:] = 0
    bs = np.shape(batch_3D_rand_ori)[0]
    ss = np.shape(batch_3D_rand_ori)[1]
    bonelen = getbone(batch_3D_rand_ori, boneindex).mean(1)     #对ss平均过的骨长，bs*16
    bonelenmean = bonelen.mean(0)   #再对bs求平均，16维向量，即每根骨头的均值
    #sample new bone lengths
    randadd = (np.random.rand(bs,len(boneindex))-0.5) * (bonelenmean * augdegree)
    bonelennew = bonelen + randadd  #随机更新过的骨长
...
class ChunkedGenerator:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, randnum, boneindex, augdegree,
        ...
        for i in range(len(poses_2d)):
            ...
            n_chunks = (poses_2d[i].shape[0] + chunk_length - 1) // chunk_length    #可以把poses_2d分成这么多部分，每部分chunk_length长，向上取整
            offset = (n_chunks * chunk_length - poses_2d[i].shape[0]) // 2          #要整除，需要在两边各补上这么多片，向下取整
    ...
    def next_epoch(self):
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):  #对batch循环
                chunks = pairs[b_i*self.batch_size : (b_i+1)*self.batch_size]   #属于该batch的chunks
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):    #对其中的某一chunks
                    ...
                    if pad_left_2d != 0 or pad_right_2d != 0:
                        self.batch_2d[i] = np.pad(seq_2d[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')    #边缘补0，方便卷积
                        
                    else:
                        self.batch_2d[i] = seq_2d[low_2d:high_2d]   #不需补0

                    if flip:    #翻转
                        # Flip 2D keypoints
                        self.batch_2d[i, :, :, 0] *= -1 #x方向翻转
                        self.batch_2d[i, :, self.kps_left + self.kps_right] = self.batch_2d[i, :, self.kps_right + self.kps_left]   #调换左右关节
                    ...
                    if self.poses_3d is not None:
                        ...
                        if pad_left_2d != 0 or pad_right_2d != 0:
                            ...
                            self.batch_3d[i] = seq_3d[low_2d:high_2d]   #同理，提取与2dchunks对应的3d真值
                    ...
                    if self.cameras is not None:
                        self.batch_cam[i] = self.cameras[seq_i] #引入相机参数
                        if flip:
                    ...
                    '''
                    #randomly sample 'randnum' frames' 2D/3D of the video   随机采样
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    '''
                    #self.batch_3d_rand[:, :, 0] = 0     #改动处，被我注释掉了
                    #first adjust the 3D gt of each frame based on the new sampled bone lengths
                    self.batch_3d_randaug, self.bonelennew = randomaug(np.copy(  self.batch_3d_rand[:len(chunks)]), self.boneindex, self.augdegree)
